...
Especially, if the address in the instruction refers user space, then we call it RDMA or Kernel Bypass. But please note the translation of physical address to virtual address should be performed on the PCI device not by OS and CPUs!
```
                    Pysical Memory
                    +----------+
                    |          |
